...
if(run){
  ...
  nsim <- 100
  result_KRF <- vector(mode = "list", length = nrow(sim_params))
  for(i in 1:nrow(sim_params)){
    cat("Iteration:", i, "of", nrow(sim_params), "\n")
    result_KRF[[i]][["parameters"]] <- sim_params[i,]
    suppressWarnings(result_KRF[[i]][["linear_gaussian"]] <- pbsapply(1:nsim, FUN = function(x) sim_linear(n = n, p = p, rho = rho, stat = stat, SNR = sim_params$SNR[i], k = sim_params$k[i])))
    suppressWarnings(result_KRF[[i]][["fs_gaussian"]] <- pbsapply(1:nsim, FUN = function(x) sim_flattened_sine(n = n, p = p, rho = rho, stat = stat, SNR = sim_params$SNR[i], k = sim_params$k[i])))
    suppressWarnings(result_KRF[[i]][["linear_fish"]] <- pbsapply(1:nsim, FUN = function(x) sim_linear_fish(SNR = sim_params$SNR[i], stat = stat, k = sim_params$k[i], p = p)))
    suppressWarnings(result_KRF[[i]][["fs_fish"]] <- pbsapply(1:nsim, FUN = function(x) sim_flattened_sine_fish(SNR = sim_params$SNR[i], stat = stat, k = sim_params$k[i], p = p)))
  }
  save(result_KRF, file = paste("C:/Users/drain/Box Sync/Mentch Coleman Shared/RF Permutation Tests/StabilizedRFPerm/knockoff_RFstat_sim_n=", n, "p=", p, "rho=", rho, ".RData", sep = ""))
}
...
if(process_results){
  ...
  clean_results <- do.call(rbind, c(lapply(result_KRF, result_summarize), make.row.names = F))
}
...
if(plot_results){
  ...
  FDR_k_plot <- ggplot(data = clean_results %>% filter(Metric == "FDR"), aes(x = k, y = Value, group = SNR)) + 
    geom_line(aes(col = SNR)) + geom_point(aes(col = SNR)) + facet_wrap(c("model_X", "model_Y")) + 
    xlab ("k") + ylab("FDR") + geom_hline(aes(yintercept = 0.10), lty = 'dashed', col = 'tomato', size = 1.2) + plot_themes
...
if(save_plots){
  ...
  FDR_k_save <- c(plot_sizes, file = c('C:/Users/drain/Box Sync/Mentch Coleman Shared/RF Permutation Tests/StabilizedRFPerm/Plots/RFKnockoff_FDR_k_plot.pdf'))
  do.call(pdf, args = FDR_k_save)
  FDR_k_plot
  dev.off()
  
  FDR_SNR_save <- c(plot_sizes, file = c('C:/Users/drain/Box Sync/Mentch Coleman Shared/RF Permutation Tests/StabilizedRFPerm/Plots/RFKnockoff_FDR_SNR_plot.pdf'))
  do.call(pdf, args = FDR_SNR_save)
  FDR_SNR_plot
  dev.off()
  
  num_selected_k_save <- c(plot_sizes, file = c('C:/Users/drain/Box Sync/Mentch Coleman Shared/RF Permutation Tests/StabilizedRFPerm/Plots/RFKnockoff_num_selected_k_plot.pdf'))
  do.call(pdf, args = num_selected_k_save)
  num_selected_k_plot
  dev.off()
  
  num_selected_SNR_save <- c(plot_sizes, file = c('C:/Users/drain/Box Sync/Mentch Coleman Shared/RF Permutation Tests/StabilizedRFPerm/Plots/RFKnockoff_num_selected_SNR_plot.pdf'))
  do.call(pdf, args = num_selected_SNR_save)
  num_selected_SNR_plot
  dev.off()
  
  power_k_save <- c(plot_sizes, file = c('C:/Users/drain/Box Sync/Mentch Coleman Shared/RF Permutation Tests/StabilizedRFPerm/Plots/RFKnockoff_power_k_plot.pdf'))
  do.call(pdf, args = power_k_save)
  power_k_plot
  dev.off()
  
  power_SNR_save <- c(plot_sizes, file = c('C:/Users/drain/Box Sync/Mentch Coleman Shared/RF Permutation Tests/StabilizedRFPerm/Plots/RFKnockoff_power_SNR_plot.pdf'))
  do.call(pdf, args = power_SNR_save)
  power_SNR_plot
  dev.off()
  
  Type_I_k_save <- c(plot_sizes, file = c('C:/Users/drain/Box Sync/Mentch Coleman Shared/RF Permutation Tests/StabilizedRFPerm/Plots/RFKnockoff_Type_I_k_plot.pdf'))
  do.call(pdf, args = Type_I_k_save)
  Type_I_k_plot
  dev.off()
  
  Type_I_SNR_save <- c(plot_sizes, file = c('C:/Users/drain/Box Sync/Mentch Coleman Shared/RF Permutation Tests/StabilizedRFPerm/Plots/RFKnockoff_Type_I_SNR_plot.pdf'))
